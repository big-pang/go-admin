...
type AdminMenu struct {
	...
	ParentId  int    `orm:"column(parent_id);size(11);default(0);description(父级菜单)" json:"parent_id"`
	Name      string `orm:"column(name);size(30);description(名称)" json:"name"`
	Url       string `orm:"column(url);size(100);index;description(url)" json:"url"`
	Icon      string `orm:"column(icon);size(30);default(fa-list);description(图标)" json:"icon"`
	IsShow    int8   `orm:"column(is_show);size(1);default(1);description(等级)" json:"is_show"`
	SortId    int    `orm:"column(sort_id);size(10);default(1000);description(排序)" json:"sort_id"`
